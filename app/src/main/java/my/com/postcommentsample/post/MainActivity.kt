...
class MainActivity : BaseActivity<PostMvp.PostView, PostPresenter>(), PostMvp.PostView {
    ...
    private fun actionBarSetting(){
        ...
        this.title = getString(R.string.post)

    }

    private fun callLoadPost(){
        presenter.loadPost()
    }
    ...
    private fun setAdapter(){
        ...
        adapter = PostAdapter { holder, post ->
            holder.root.setOnClickListener { showToast(post.getId().toString()) }
        }
    ...
    override fun getPostLoaded(list: List<Post>) {
        adapter.setPosts(list)
    }

    override fun doneLoadPost() {
        hideLoading()
    }
